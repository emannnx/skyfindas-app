          ...
          {weeks.map((week, weekIndex) => (
            <React.Fragment key={weekIndex}>
              {week.map((day, dayIndex) => {
                if (!day) {
                  return <div key={`empty-${dayIndex}`} className="calendar-day" style={{ visibility: 'hidden' }}></div>;
                }
                ...
                return (
                  <div
                    key={day.toString()}
                    className={`calendar-day ${isSelected ? 'selected' : ''} ${hasAppointments ? 'has-appointments' : ''}`}

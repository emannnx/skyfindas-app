...
const Analytics = () => {
  ...
  const calculateStats = () => {
    ...
    const stats = {
      ...
      approvedAppointments: appointments.filter(app => app.status === 'Approved').length,
    ...
    <div className="container" style={{ padding: '40px 20px' }}>
      ...
      <div style={{ 
        ...
        <button
          ...
          className={`btn ${timeRange === 'today' ? 'btn-primary' : 'btn-outline'}`}
        >
          Today
        </button>
        <button
          ...
          {Object.entries(statusData).map(([status, count]) => (
            <div key={status} style={{ 
              ...
              <div style={{ 
                fontSize: '24px', 
                fontWeight: 'bold',
                color: status === 'Approved' ? 'var(--success-color)' :
                       status === 'Pending' ? 'var(--warning-color)' : 'var(--danger-color)'
              }}>
                {count}
              </div>
              <div style={{ 
              ...
              <div style={{ 
                ...
                {stats.totalAppointments > 0 
                  ? `${((count / stats.totalAppointments) * 100).toFixed(1)}% of total`
                  : '0%'
                }
              </div>
            </div>
                ...
                <tbody>
                  {serviceData.map((service) => (
                    <tr key={service.name}>
                      ...
                      <td>
                        <span style={{ 
                          color: service.total > 0 
                            ? ((service.approved / service.total) * 100) > 70 
                              ? 'var(--success-color)' 
                              : ((service.approved / service.total) * 100) > 50 
                                ? 'var(--warning-color)' 
                                : 'var(--danger-color)'
                            : 'var(--gray-color)',
                          fontWeight: '600'
                          ...
                          {service.total > 0 
                            ? `${((service.approved / service.total) * 100).toFixed(1)}%`
                            : 'N/A'
                          }
                        </span>
                      </td>
